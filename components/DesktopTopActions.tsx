...
export default function DesktopTopActions() {
  ...
  return (
    <div className="hidden md:flex  items-center justify-end gap-3 pt-20">
      {items.map((item) => {
        ...
        return (
          <Link
            key={item.label}
            href={item.href}
            className={`flex items-center gap-2 rounded-full px-4 py-2 transition border border-${color}/15 ${item.href === pathname
